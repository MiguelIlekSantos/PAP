...
export const CreateBudgetDto = Joi.object({
  ...
  usedAmount: Joi.number().min(0).required().messages({
    'number.base': 'Used amount must be a number',
    'number.min': 'Used amount must be at least 0',
    'any.required': 'Used amount is required'
  }),
  remainingAmount: Joi.number().min(0).required().messages({
  ...
  subDepartmentId: Joi.number().integer().positive().optional().messages({
    'number.base': 'Sub department ID must be a number',
    'number.integer': 'Sub department ID must be an integer',
    'number.positive': 'Sub department ID must be positive'
  }),
  ...
  campaignId: Joi.number().integer().positive().optional().messages({
  ...
  saleId: Joi.number().integer().positive().optional().messages({
...
export const UpdateBudgetDto = Joi.object({
  name: Joi.string().optional(),
  amount: Joi.number().positive().optional().messages({
    'number.base': 'Amount must be a number',
    'number.positive': 'Amount must be positive'
  }),
  usedAmount: Joi.number().min(0).optional().messages({
    'number.base': 'Used amount must be a number',
    'number.min': 'Used amount must be at least 0'
  }),
...
export const DeleteBudgetDto = Joi.object({
