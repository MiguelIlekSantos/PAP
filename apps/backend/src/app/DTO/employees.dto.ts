...
export const CreateEmployeesDto = Joi.object({
  ...
  dateOfHire: Joi.date().required().messages({
    'date.base': 'Date of hire must be a valid date',
    'any.required': 'Date of hire is required'
  }),
  shiftType: Joi.string().required().messages({
    'string.empty': 'Shift type is required',
    'any.required': 'Shift type is required'
  }),
  workingHours: Joi.number().integer().min(0).max(24).required().messages({
    ...
    'any.required': 'Working hours is required'
  }),
  workingDays: Joi.number().integer().min(0).max(7).required().messages({
...
export const UpdateEmployeesDto = Joi.object({
  ...
  dateOfHire: Joi.date().optional().messages({
    'date.base': 'Date of hire must be a valid date'
  }),
...
export const DeleteEmployeesDto = Joi.object({
